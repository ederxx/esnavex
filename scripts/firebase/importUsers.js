...
/*
  Usage:
    node importUsers.js --users=./data/users.json [--hash-algo=BCRYPT|SCRYPT]

  users.json: array of user objects.
  Two main modes:
    - If user objects include `passwordHash` (base64) and you pass --hash-algo, the script will use Admin SDK's importUsers with hash options.
    - Otherwise the script will create users without passwords and print password reset links.

  Example users.json items:
  {
    "uid": "abc123",
  ...
  Notes on hashing: for BCRYPT you only need passwordHash (base64). For SCRYPT you need passwordHash and passwordSalt and additional options.
*/
...
(async function main() {
  try {
    ...
    if (hasHashes && hashAlgo) {
      ...
      // Basic support for BCRYPT and SCRYPT. For SCRYPT you may need to provide additional params and convert to Buffers.
